...
class Product {
  ...
  static _getImageList(Map<String, dynamic> json) {
    List<XShopImage> imageList = [];
    if (json != null && json['edges'] != null)
      json['edges'].forEach((image) => imageList.add(XShopImage.fromJson(image['node'] ?? const {})));
    return imageList;
  }
...
class XShopImage {
  ...
  const XShopImage({this.altText, this.originalSource, this.id});

  static XShopImage fromJson(Map<String, dynamic> json) {
    return XShopImage(
